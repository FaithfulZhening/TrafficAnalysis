...
def analysis_host_rtt(top_three_host_pair):
    ...
    for (number, eths_list) in top_three_host_pair:
        ...
        print(inet_to_str(number[0]))
        print(inet_to_str(number[1]))
        print(median_rtt_start_time_pair)
        if median_rtt_start_time_pair:
            median_rtt_start_time_pair.sort(key=lambda x: x[1])
            plot_host_rtt_function(median_rtt_start_time_pair,"start time","representative rtt (ms)","Host RTT Analysis")